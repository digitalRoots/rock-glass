...
desc "Display usage"
task :help do
  puts " $ rake post title=\"A Title\" [date=\"YYYY-MM-DD\"] [tags=[tag1, tag2]] [category=\"category\"]"
  puts " $ rake page [name=\"page.md\"]"
  puts " $ rake preview"
  puts " - theme not yet supported"
end
...
task :preview do
  system "bundler exec jekyll serve --watch"
end # task :preview
